...
@section('content')
<x-common.page-breadcrumb pageTitle="Data Produk" />
  ...
  <div class="grid grid-cols-12 gap-4 md:gap-6">
    ...
    <div class="col-span-12" x-data='{
      ...
      get filteredProduks() {
        ...
        return this.produks.filter(produk => {
          ...
          return produk.kode_produk.toLowerCase().includes(query) ||
                 produk.nama_produk.toLowerCase().includes(query);
        });
      },
      get sortedProduks() {
        if (!this.sortField) return this.filteredProduks;
        return [...this.filteredProduks].sort((a, b) => {
      ...
      get totalPages() {
        return Math.ceil(this.sortedProduks.length / this.itemsPerPage);
      },
      get paginatedProduks() {
        const start = (this.currentPage - 1) * this.itemsPerPage;
        const end = start + this.itemsPerPage;
        return this.sortedProduks.slice(start, end);
      },
      get displayPages() {
        const pages = [];
        const maxVisible = 5;
      ...
      confirmDelete() {
        ...
        fetch(`/data-produk/${this.deleteTarget.id}`, {
          method: "DELETE",
          headers: {
        ...
        .then(data => {
          if (data.success) {
            this.produks = this.produks.filter(p => p.id !== this.deleteTarget.id);
    ...
    <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('data-produk.print') }}" target="_blank" class="inline-flex items-center justify-center gap-2 rounded-lg bg-gray-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-gray-600">
                ...
                <a href="{{ route('data-produk.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-blue-600">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <table class="w-full min-w-[1200px]">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400">Foto Produk</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400 cursor-pointer" @click="sortBy('kode_produk')">Kode Produk</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400 cursor-pointer" @click="sortBy('nama_produk')">Nama Produk</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400">Harga Beli</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400">Harga Jual</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400">Standar Stok</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400 cursor-pointer" @click="sortBy('aktual_stok')">Aktual Stok</th>
                        <th class="px-4 py-3 text-left text-sm font-medium text-gray-500 dark:text-gray-400">Status Stok</th>
                        <th class="px-4 py-3 text-center text-sm font-medium text-gray-500 dark:text-gray-400">Action</th>
                    </tr>
                </thead>
                <tbody>
                    <template x-for="(produk, index) in paginatedProduks" :key="produk.id">
