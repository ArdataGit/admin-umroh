...
@section('content')
<x-common.page-breadcrumb pageTitle="Manifest Jamaah Haji" :breadcrumbs="[
...
<div class="space-y-6">
    ...
    <div class="max-w-5xl rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        <h3 class="mb-4 text-xl font-semibold text-gray-800 dark:text-white">Informasi Keberangkatan Haji</h3>
            ...
            <div class="space-y-6">
                ...
                <div class="border-b border-gray-100 pb-3 dark:border-gray-700">
                    ...
                    <div class="font-semibold text-gray-800 dark:text-white">
                        {{ $keberangkatan->paketHaji->kode_paket }} - {{ $keberangkatan->nama_keberangkatan }}
                    </div>
                </div>
                ...
                <div class="border-b border-gray-100 pb-3 dark:border-gray-700">
                    ...
                    <div class="font-semibold text-gray-800 dark:text-white">
                        {{ $keberangkatan->paketHaji->maskapai->nama_maskapai ?? '-' }} <span class="text-gray-400 mx-1">/</span> <span class="uppercase">{{ $keberangkatan->paketHaji->rute_penerbangan ?? '-' }}</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        @if($keberangkatan->paketHaji->harga_quad_1 > 0)
                        <div>
                            <div class="text-xs text-gray-500">Quad:</div>
                            <div class="font-medium text-gray-800 dark:text-white">Rp {{ number_format($keberangkatan->paketHaji->harga_quad_1, 0, ',', '.') }}</div>
                        </div>
                        @endif
                        @if($keberangkatan->paketHaji->harga_triple_1 > 0)
                        <div>
                            <div class="text-xs text-gray-500">Triple:</div>
                            <div class="font-medium text-gray-800 dark:text-white">Rp {{ number_format($keberangkatan->paketHaji->harga_triple_1, 0, ',', '.') }}</div>
                        </div>
                        @endif
                        @if($keberangkatan->paketHaji->harga_double_1 > 0)
                        <div>
                            <div class="text-xs text-gray-500">Double:</div>
                            <div class="font-medium text-gray-800 dark:text-white">Rp {{ number_format($keberangkatan->paketHaji->harga_double_1, 0, ',', '.') }}</div>
                        </div>
                        @endif
                    </div>
                </div>
    ...
    <div class="max-w-5xl rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Daftar Jamaah Haji</h3>
            <a href="{{ route('customer-haji.create', $keberangkatan->id) }}" class="rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-600">
                + Tambah Jamaah
            </a>
        </div>
        ...
        <div class="overflow-x-auto">
             <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400 whitespace-nowrap">
                ...
                <tbody>
                    @forelse($customerHajis as $index => $customer)
                    <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('customer-haji.show', $customer->id) }}" class="text-gray-500 hover:text-gray-700" title="Lihat">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z" />
                                    </svg>
                                </a>
                                <a href="{{ route('customer-haji.edit', $customer->id) }}" class="text-blue-500 hover:text-blue-700" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                    </svg>
                                </a>
                                <form action="{{ route('customer-haji.destroy', $customer->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data jamaah haji ini?');" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                        <td colspan="25" class="px-6 py-8 text-center text-gray-500">
                            Belum ada data jamaah haji. Silakan tambahkan jamaah.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
