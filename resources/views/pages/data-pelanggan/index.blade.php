...
@section('content')
<x-common.page-breadcrumb pageTitle="Data Pelanggan" />
  ...
  <div class="grid grid-cols-12 gap-4 md:gap-6">
    ...
    <div class="col-span-12" x-data='{
      ...
      get filteredPelanggans() {
        if (!this.searchQuery) return this.pelanggans;
        return this.pelanggans.filter(pelanggan => {
          const query = this.searchQuery.toLowerCase();
          return pelanggan.kode_pelanggan.toLowerCase().includes(query) ||
                 pelanggan.nama_pelanggan.toLowerCase().includes(query) ||
                 pelanggan.email_pelanggan.toLowerCase().includes(query);
        });
      },
      get sortedPelanggans() {
        if (!this.sortField) return this.filteredPelanggans;
        return [...this.filteredPelanggans].sort((a, b) => {
      ...
      get totalPages() {
        return Math.ceil(this.sortedPelanggans.length / this.itemsPerPage);
      },
      get paginatedPelanggans() {
        const start = (this.currentPage - 1) * this.itemsPerPage;
        const end = start + this.itemsPerPage;
        return this.sortedPelanggans.slice(start, end);
      },
      get displayPages() {
        const pages = [];
        const maxVisible = 5;
      ...
      confirmDelete() {
        ...
        fetch(`/data-pelanggan/${this.deleteTarget.id}`, {
          method: "DELETE",
          headers: {
        ...
        .then(data => {
          if (data.success) {
            this.pelanggans = this.pelanggans.filter(p => p.id !== this.deleteTarget.id);
    ...
    <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('data-pelanggan.print') }}" target="_blank" class="inline-flex items-center justify-center gap-2 rounded-lg bg-gray-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-gray-600">
                ...
                <a href="{{ route('data-pelanggan.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-blue-600">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <table class="w-full min-w-[1200px]">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('kode_pelanggan')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Kode Pelanggan</p>
                                <svg class="w-4 h-4" :class="sortField === 'kode_pelanggan' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('nama_pelanggan')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Pelanggan</p>
                                <svg class="w-4 h-4" :class="sortField === 'nama_pelanggan' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('kontak_pelanggan')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Kontak</p>
                                <svg class="w-4 h-4" :class="sortField === 'kontak_pelanggan' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                        ...
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('status_pelanggan')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Status</p>
                                <svg class="w-4 h-4" :class="sortField === 'status_pelanggan' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                ...
                <tbody>
                    <template x-for="(pelanggan, index) in paginatedPelanggans" :key="pelanggan.id">
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4">
                                <div class="h-10 w-10 overflow-hidden rounded-full">
                                    <img :src="pelanggan.foto_pelanggan ? '/storage/' + pelanggan.foto_pelanggan : 'https://ui-avatars.com/api/?name=' + pelanggan.nama_pelanggan + '&background=random'" alt="User" class="h-full w-full object-cover"/>
                                </div>
                            </td>
                            <td class="px-4 py-4">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90" x-text="pelanggan.kode_pelanggan"></p>
                            </td>
                            <td class="px-4 py-4">
                                <p class="text-gray-800 text-theme-sm dark:text-gray-400" x-text="pelanggan.nama_pelanggan"></p>
                            </td>
                             <td class="px-4 py-4">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="pelanggan.kontak_pelanggan"></p>
                            </td>
                            <td class="px-4 py-4">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="pelanggan.kabupaten_kota"></p>
                            </td>
                            <td class="px-4 py-4">
                                <span :class="pelanggan.status_pelanggan === 'Active' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400'" class="inline-flex rounded-full px-2 py-1 text-xs font-medium">
                                    <span x-text="pelanggan.status_pelanggan"></span>
                                </span>
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex justify-center gap-2">
                                    <a :href="`/data-pelanggan/${pelanggan.id}`" title="View" class="inline-flex items-center justify-center rounded-lg bg-green-500 p-2 text-white hover:bg-green-600">
                                    ...
                                    <a :href="`/data-pelanggan/${pelanggan.id}/edit`" title="Edit" class="inline-flex items-center justify-center rounded-lg bg-blue-500 p-2 text-white hover:bg-blue-600">
                                        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <button @click="openDeleteModal(pelanggan.id, pelanggan.nama_pelanggan)" title="Delete" class="inline-flex items-center justify-center rounded-lg bg-red-500 p-2 text-white hover:bg-red-600">
                                        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
