...
@section('content')
<x-common.page-breadcrumb pageTitle="Pembelian Produk" :breadcrumbs="[
    ['label' => 'Pembelian Produk', 'url' => '#']
]" />
    ...
    <div class="col-span-12" x-data='{
      ...
      get filteredItems() {
        ...
        return this.items.filter(i => {
          return i.kode_pembelian.toLowerCase().includes(query) ||
                 i.supplier?.nama_supplier.toLowerCase().includes(query);
        });
      },
      get paginatedItems() {
      ...
      confirmDelete() {
        ...
        fetch(`/pembelian-produk/${this.deleteTarget.id}`, {
          method: "DELETE",
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            <div class="flex flex-col gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-700 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Riwayat Pembelian</h3>
                    @if($canCreate)
                    <a href="{{ route('pembelian-produk.create') }}" class="rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-600">
                        Tambah Pembelian
                    </a>
                    @endif
                </div>
                    ...
                    <tbody>
                        <template x-for="(item, index) in paginatedItems" :key="item.id">
                            <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4" x-text="new Date(item.tanggal_pembelian).toLocaleDateString('id-ID')"></td>
                                <td class="px-6 py-4 font-medium" x-text="item.kode_pembelian"></td>
                                <td class="px-6 py-4" x-text="item.supplier?.nama_supplier"></td>
                                <td class="px-6 py-4 text-center" x-text="item.details?.reduce((acc, curr) => acc + curr.quantity, 0) || 0"></td>
                                <td class="px-6 py-4 font-bold" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(item.total_pembayaran)"></td>
                                <td class="px-6 py-4" x-text="'Rp ' + new Intl.NumberFormat('id-ID').format(item.jumlah_bayar)"></td>
                                <td class="px-6 py-4 uppercase" x-text="item.metode_pembayaran"></td>
                                <td class="px-6 py-4">
                                     <span :class="{
                                        'bg-blue-100 text-blue-800': item.status_pembayaran === 'order',
                                        'bg-yellow-100 text-yellow-800': item.status_pembayaran === 'delivery',
                                        'bg-green-100 text-green-800': item.status_pembayaran === 'completed'
                                     }" class="px-2 py-1 rounded text-xs font-semibold uppercase" x-text="item.status_pembayaran"></span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a :href="`/pembelian-produk/${item.id}`" class="text-gray-500 hover:text-gray-700" title="Lihat">
                                        ...
                                        @if($canEdit)
                                        <a :href="`/pembelian-produk/${item.id}/edit`" class="text-blue-500 hover:text-blue-700" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        ...
                                        @endif
                                        @if($canDelete)
                                        <button @click="openDeleteModal(item.id, item.kode_pembelian)" class="text-red-500 hover:text-red-700" title="Hapus">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                         ...
                         <tr x-show="paginatedItems.length === 0">
                            <td colspan="10" class="px-6 py-4 text-center text-gray-500">Belum ada data pembelian.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" x-cloak>
                <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                    <h3 class="text-lg font-bold">Hapus Pembelian?</h3>
                    <p>Hapus pembelian <span x-text="deleteTarget?.kode" class="font-bold"></span>?</p>
                    <p class="text-sm text-red-500 mt-2">Jika status 'Completed', stok produk yang dibeli akan dikembalikan (dikurangi).</p>
