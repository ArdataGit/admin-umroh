...
@section('content')
<x-common.page-breadcrumb pageTitle="Tambah Pembelian Produk" :breadcrumbs="[
    ['label' => 'Pembelian Produk', 'url' => route('pembelian-produk.index')],
    ['label' => 'Tambah', 'url' => '#']
]" />

<div class="grid grid-cols-12 gap-4 md:gap-6">
    <div class="col-span-12" x-data="pembelianProduk()">
        <form @submit.prevent="submitForm">
        
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            <h3 class="mb-6 text-xl font-semibold text-gray-800 dark:text-white">Form Pembelian Produk</h3>
                ...
                <div class="bg-gray-50 dark:bg-gray-800 rounded-lg p-6 space-y-3">
                    ...
                    <div class="pt-4 border-t border-gray-200 dark:border-gray-700">
                         <div class="flex justify-between items-center text-sm text-gray-600 dark:text-gray-400 mb-2">
                            ...
                            <input type="text" 
                                   :value="formatNumber(form.jumlah_bayar)" 
                                   @input="let val = $el.value.replace(/\D/g, ''); form.jumlah_bayar = val === '' ? 0 : parseInt(val); $el.value = formatNumber(form.jumlah_bayar); calculateChange()" 
                                   class="w-32 text-right rounded border border-gray-300 px-2 py-1 text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200 font-bold" placeholder="0" />
                        </div>
            ...
            <div class="mt-8 flex justify-end gap-3">
                ...
                <button type="submit" :disabled="form.details.length === 0" class="rounded-lg bg-blue-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-blue-600 disabled:opacity-50 disabled:cursor-not-allowed">Simpan Pembelian</button>
            </div>
        </div>
        </form>
    </div>
...
<script>
    function pembelianProduk() {
        return {
            ...
            submitForm() {
                fetch('{{ route('pembelian-produk.store') }}', {
                    method: 'POST',
                    headers: {
