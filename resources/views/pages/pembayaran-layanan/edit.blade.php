...
@section('content')
<x-common.page-breadcrumb pageTitle="Edit Pembayaran Layanan" :breadcrumbs="[
    ...
    <div class="col-span-12">
        <div class="rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            <div class="border-b border-gray-100 px-6 py-5 dark:border-gray-800">
                <h3 class="text-base font-semibold text-gray-800 dark:text-white">Edit Pembayaran - {{ $pembayaran->transaksiLayanan->pelanggan->nama_pelanggan ?? 'Umum' }}</h3>
                <p class="text-sm text-gray-500">Kode Transaksi: {{ $pembayaran->kode_transaksi }} | Kode Trx Layanan: {{ $pembayaran->transaksiLayanan->kode_transaksi }}</p>
            </div>
            
            <form action="{{ route('pembayaran-layanan.update', $pembayaran->id) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Tanggal Pembayaran -->
                     <div>
                        ...
                        <input type="date" name="tanggal_pembayaran" value="{{ old('tanggal_pembayaran', $pembayaran->tanggal_pembayaran) }}" 
                    ...
                    <div>
                        ...
                        <select name="metode_pembayaran" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:text-white" required>
                            ...
                            @foreach(['cash', 'transfer', 'debit', 'qris', 'other'] as $method)
                                <option value="{{ $method }}" {{ old('metode_pembayaran', $pembayaran->metode_pembayaran) == $method ? 'selected' : '' }}>
                                    {{ ucfirst($method) }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="flex justify-end gap-3 border-t border-gray-100 pt-6 dark:border-gray-800">
                    <a href="{{ route('pembayaran-layanan.index') }}" class="rounded-lg border border-gray-300 bg-white px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-400 dark:hover:bg-gray-800">Batal</a>
                    <button type="submit" class="rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-blue-700">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
