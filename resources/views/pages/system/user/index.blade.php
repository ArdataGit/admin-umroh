...
@section('content')
    <x-common.page-breadcrumb pageTitle="User Management" :breadcrumbs="[['label' => 'System', 'url' => '#'], ['label' => 'User', 'url' => '#']]" />
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="mt-1 text-2xl font-bold text-gray-800 dark:text-white/90">{{ $users->count() }}</h3>
                </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="mt-1 text-2xl font-bold text-green-600">{{ $users->where('status', 'active')->count() }}</h3>
                </div>
                <div class="flex h-12 w-12 items-center justify-center rounded-lg bg-green-50 text-green-600 dark:bg-green-500/10 dark:text-green-500">
                    <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div x-data="{
        ...
        get filteredItems() {
            ...
            return this.items.filter(item => {
                ...
                return item.name.toLowerCase().includes(this.searchQuery.toLowerCase()) ||
                       roleName.includes(this.searchQuery.toLowerCase());
            });
        },
        get paginatedItems() {
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between p-6 gap-4 border-b border-gray-100 dark:border-gray-800">
            <div class="relative w-full md:w-96">
                <input x-model="searchQuery" type="text" placeholder="Cari nama, email, atau role..."
            ...
            <div class="flex items-center gap-3">
                <button @click="showRoleModal = true" class="inline-flex items-center justify-center gap-2 rounded-lg bg-white border border-gray-200 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800">
                    <template x-for="item in paginatedItems" :key="item.id">
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap font-medium text-gray-900 dark:text-white" x-text="item.name"></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 capitalize dark:bg-blue-900/30 dark:text-blue-400" x-text="item.role ? item.role.name : '-'"></span>
                            </td>
                ...
                <div x-show="showRoleModal" 
                    ...
                    <form action="{{ route('role.store') }}" method="POST">
                        ...
                        <div class="px-6 py-6 pb-4">
                            ...
                            <div class="mt-4">
                                <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Nama Role <span class="text-red-500">*</span></label>
                                <input type="text" name="name" id="name" required class="w-full rounded-lg border border-gray-200 bg-transparent px-4 py-2.5 text-sm outline-none transition-colors focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700 dark:text-white" placeholder="Bisa menggunakan huruf kecil, cth: manager">
                                <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">Role ini nantinya dapat dipilih saat menambahkan pengguna baru.</p>
                            </div>
                        </div>
