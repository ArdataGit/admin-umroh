...
@section('content')
<x-common.page-breadcrumb pageTitle="Stock Opname" :breadcrumbs="[
    ...
    <div class="col-span-12" x-data='{
      ...
      get filteredItems() {
        ...
        return this.items.filter(i => {
          return i.kode_adjustment.toLowerCase().includes(query) ||
                 i.produk?.nama_produk.toLowerCase().includes(query);
        });
      },
      get paginatedItems() {
      ...
      confirmDelete() {
        ...
        fetch(`/stock-opname/${this.deleteTarget.id}`, {
          method: "DELETE",
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            <div class="flex flex-col gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-700 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Riwayat Stock Opname</h3>
                    @if($canCreate)
                    <a href="{{ route('stock-opname.create') }}" class="rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-600">
                        Tambah Stock Opname
                    </a>
                    @endif
                </div>
                ...
                <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th class="px-6 py-3">Stock Awal</th>
                            <th class="px-6 py-3">Koreksi</th>
                            <th class="px-6 py-3">Stock Akhir</th>
                    ...
                    <tbody>
                        <template x-for="(item, index) in paginatedItems" :key="item.id">
                            <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4" x-text="new Date(item.tanggal_adjustment).toLocaleDateString('id-ID')"></td>
                                <td class="px-6 py-4 font-medium" x-text="item.kode_adjustment"></td>
                                <td class="px-6 py-4">
                                     <span :class="{
                                        'bg-green-100 text-green-800': item.tipe_adjustment === 'penambahan',
                                        'bg-red-100 text-red-800': item.tipe_adjustment === 'pengurangan'
                                     }" class="px-2 py-1 rounded text-xs font-semibold uppercase" x-text="item.tipe_adjustment"></span>
                                </td>
                                <td class="px-6 py-4" x-text="item.produk?.nama_produk"></td>
                                <td class="px-6 py-4" x-text="item.stok_awal || '-'"></td>
                                <td class="px-6 py-4 font-bold" x-text="item.koreksi_stock"></td>
                                <td class="px-6 py-4" x-text="item.stok_akhir || '-'"></td>
                                <td class="px-6 py-4" x-text="item.user_id || 'System'"></td>
                                <td class="px-6 py-4">
                                    <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs font-semibold uppercase" x-text="item.status_approval || 'Approved'"></span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a :href="`/stock-opname/${item.id}`" class="text-gray-500 hover:text-gray-700" title="Lihat">
                                        ...
                                        <template x-if="canEdit">
                                            <a :href="`/stock-opname/${item.id}/edit`" class="text-blue-500 hover:text-blue-700" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                        ...
                                        <template x-if="canDelete">
                                            <button @click="openDeleteModal(item.id, item.kode_adjustment)" class="text-red-500 hover:text-red-700" title="Hapus">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                         ...
                         <tr x-show="paginatedItems.length === 0">
                            <td colspan="11" class="px-6 py-4 text-center text-gray-500">Belum ada data Stock Opname.</td>
                        </tr>
                    </tbody>
                </table>
            </div>
            ...
            <!-- Delete Modal -->
             <div x-show="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" x-cloak>
                <div class="bg-white p-6 rounded-lg shadow-lg w-96">
                    ...
                    <p>Hapus Stock Opname <span x-text="deleteTarget?.kode" class="font-bold"></span>?</p>
                    <p class="text-sm text-red-500 mt-2">Stok produk akan dikembalikan (reversed).</p>
