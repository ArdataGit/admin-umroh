...
@section('content')
<x-common.page-breadcrumb pageTitle="Data Tabungan Umroh" />
  ...
  <div class="grid grid-cols-12 gap-4 md:gap-6">
    <div class="col-span-12" x-data='{
      ...
      confirmDelete() {
        ...
        fetch(`/tabungan-umroh/${this.deleteTarget.id}`, {
          method: "DELETE",
                ...
                <tbody>
                    <template x-for="(tabungan, index) in paginatedTabungans" :key="tabungan.id">
                        <tr class="border-b border-gray-100 hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800 text-sm text-gray-800 dark:text-white">
                            ...
                            <td class="px-4 py-4">
                                <a :href="`/setoran-umroh/${tabungan.id}`" class="text-blue-500 hover:text-blue-700 hover:underline" x-text="tabungan.jamaah?.nama_lengkap || '-'"></a>
                            </td>
                            <td class="px-4 py-4" x-text="tabungan.jamaah?.kontak_jamaah || '-'"></td>
                            <td class="px-4 py-4">Rp 0</td> <!-- Placeholder for Total Penarikan -->
                            <td class="px-4 py-4" x-text="new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR' }).format(tabungan.setoran_tabungan)"></td>
                            <td class="px-4 py-4">Umroh</td>
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                     <a :href="`/tabungan-umroh/${tabungan.id}`" class="text-gray-500 hover:text-green-600 dark:text-gray-400 dark:hover:text-green-500" title="View">
                                    ...
                                    @if($canEdit)
                                    <a :href="`/tabungan-umroh/${tabungan.id}/edit`" class="text-gray-500 hover:text-blue-600 dark:text-gray-400 dark:hover:text-blue-500" title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
