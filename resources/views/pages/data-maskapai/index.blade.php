...
@section('content')
<x-common.page-breadcrumb pageTitle="Data Maskapai" />
  ...
  <div class="grid grid-cols-12 gap-4 md:gap-6">
    ...
    <div class="col-span-12" x-data='{
      ...
      deleteTarget: null,
      get filteredMaskapais() {
        if (!this.searchQuery) return this.maskapais;
        return this.maskapais.filter(maskapai => {
          const query = this.searchQuery.toLowerCase();
          return maskapai.kode_maskapai.toLowerCase().includes(query) ||
                 maskapai.nama_maskapai.toLowerCase().includes(query) ||
                 maskapai.rute_penerbangan.toLowerCase().includes(query);
        });
      },
      get sortedMaskapais() {
        if (!this.sortField) return this.filteredMaskapais;
        return [...this.filteredMaskapais].sort((a, b) => {
      ...
      get totalPages() {
        return Math.ceil(this.sortedMaskapais.length / this.itemsPerPage);
      },
      get paginatedMaskapais() {
        const start = (this.currentPage - 1) * this.itemsPerPage;
        const end = start + this.itemsPerPage;
        return this.sortedMaskapais.slice(start, end);
      },
      get displayPages() {
        const pages = [];
        const maxVisible = 5;
      ...
      confirmDelete() {
        ...
        fetch(`/data-maskapai/${this.deleteTarget.id}`, {
          method: "DELETE",
          headers: {
        ...
        .then(data => {
          if (data.success) {
            this.maskapais = this.maskapais.filter(m => m.id !== this.deleteTarget.id);
    ...
    <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('data-maskapai.print') }}" target="_blank" class="inline-flex items-center justify-center gap-2 rounded-lg bg-gray-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-gray-600">
                ...
                <a href="{{ route('data-maskapai.export') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-green-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-green-600">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <a href="{{ route('data-maskapai.create') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-blue-500 px-4 py-2.5 text-sm font-medium text-white shadow-theme-xs hover:bg-blue-600">
                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div class="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="max-w-full overflow-x-auto custom-scrollbar">
            <table class="w-full min-w-[1200px]">
                    ...
                    <tr class="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('kode_maskapai')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Kode Maskapai</p>
                                <svg class="w-4 h-4" :class="sortField === 'kode_maskapai' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('nama_maskapai')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Nama Maskapai</p>
                                <svg class="w-4 h-4" :class="sortField === 'nama_maskapai' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('rute_penerbangan')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Rute Penerbangan</p>
                                <svg class="w-4 h-4" :class="sortField === 'rute_penerbangan' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('lama_perjalanan')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Lama Perjalanan</p>
                                <svg class="w-4 h-4" :class="sortField === 'lama_perjalanan' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-4 py-3 text-left cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800" @click="sortBy('harga_tiket')">
                            <div class="flex items-center gap-1">
                                <p class="font-medium text-gray-500 text-theme-xs dark:text-gray-400">Harga Tiket</p>
                                <svg class="w-4 h-4" :class="sortField === 'harga_tiket' && sortDirection === 'asc' ? 'text-blue-500' : 'text-gray-400'" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"/>
                                </svg>
                            </div>
                        </th>
                ...
                <tbody>
                    <template x-for="(maskapai, index) in paginatedMaskapais" :key="maskapai.id">
                        <tr class="border-b border-gray-100 dark:border-gray-800">
                            <td class="px-4 py-4">
                            ...
                            <td class="px-4 py-4">
                                <p class="text-gray-800 text-theme-sm dark:text-white/90" x-text="maskapai.nama_maskapai"></p>
                            </td>
                            <td class="px-4 py-4">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="maskapai.rute_penerbangan"></p>
                            </td>
                            <td class="px-4 py-4">
                                <p class="text-gray-500 text-theme-sm dark:text-gray-400" x-text="maskapai.lama_perjalanan + ' Jam'"></p>
                            </td>
                            <td class="px-4 py-4">
                                <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90" x-text="formatPrice(maskapai.harga_tiket)"></p>
                            </td>
                            <td class="px-4 py-4">
                                <div class="flex justify-center gap-2">
                                    <a :href="`/data-maskapai/${maskapai.id}`" title="View" class="inline-flex items-center justify-center rounded-lg bg-green-500 p-2 text-white hover:bg-green-600">
                                    ...
                                    <a :href="`/data-maskapai/${maskapai.id}/edit`" title="Edit" class="inline-flex items-center justify-center rounded-lg bg-blue-500 p-2 text-white hover:bg-blue-600">
                                        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <button @click="openDeleteModal(maskapai.id, maskapai.nama_maskapai)" title="Delete" class="inline-flex items-center justify-center rounded-lg bg-red-500 p-2 text-white hover:bg-red-600">
                                        <svg width="16" height="16" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
