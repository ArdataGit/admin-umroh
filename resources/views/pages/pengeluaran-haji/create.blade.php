...
@section('content')
<x-common.page-breadcrumb pageTitle="Tambah Pengeluaran Haji" :breadcrumbs="[
    ['label' => 'Pengeluaran Haji', 'url' => route('pengeluaran-haji.index')],
    ['label' => 'Tambah', 'url' => '#']
]" />
...
<div class="grid grid-cols-12 gap-4 md:gap-6">
    <div class="col-span-12" x-data="{
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            ...
            <form action="{{ route('pengeluaran-haji.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    
                    <!-- Keberangkatan (Dropdown) -->
                    <div class="md:col-span-2">
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Keberangkatan Haji</label>
                        <select name="keberangkatan_haji_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 focus:border-blue-500 focus:outline-none dark:border-gray-700 dark:bg-gray-900 dark:text-white" required>
                            <option value="">-- Pilih Keberangkatan --</option>
                            @foreach($keberangkatans as $k)
                                <option value="{{ $k->id }}">{{ $k->kode_keberangkatan }} - {{ $k->nama_keberangkatan }} ({{ $k->paketHaji->nama_paket }})</option>
                            @endforeach
                        </select>
                         @error('keberangkatan_haji_id') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                ...
                <div class="mt-8 flex justify-end gap-3">
                    <a href="{{ route('pengeluaran-haji.index') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</a>
