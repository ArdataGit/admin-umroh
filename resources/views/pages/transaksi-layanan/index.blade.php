...
@section('content')
<x-common.page-breadcrumb pageTitle="Transaksi Layanan" :breadcrumbs="[
    ['label' => 'Transaksi Layanan', 'url' => '#']
]" />
...
<div class="grid grid-cols-12 gap-4 md:gap-6">
    <div class="col-span-12" x-data='{
      ...
      confirmDelete() {
        ...
        fetch(`/transaksi-layanan/${this.deleteTarget.id}`, {
          method: "DELETE",
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            <div class="flex flex-col gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-700 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-4">
                    ...
                    @if($canCreate)
                    <a href="{{ route('transaksi-layanan.create') }}" class="rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-600">
                        Tambah Transaksi
                    </a>
                    @endif
                </div>
                ...
                <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-center">Jumlah Layanan</th>
                    ...
                    <tbody>
                        <template x-for="(item, index) in paginatedItems" :key="item.id">
                            <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4" x-text="new Date(item.tanggal_transaksi).toLocaleDateString('id-ID')"></td>
                                ...
                                <td class="px-6 py-4">
                                     <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">
                                         <span x-text="item.pembayaran_layanans?.length > 0 ? [...new Set(item.pembayaran_layanans.map(p => p.metode_pembayaran))].join(', ') : '-'"></span>
                                     </span>
                                </td>
                                <td class="px-6 py-4">
                                     <span :class="{
                                ...
                                <td class="px-6 py-4">
                                    <span :class="{
                                        'bg-green-100 text-green-800': (item.total_transaksi - (item.pembayaran_layanans?.reduce((acc, curr) => acc + Number(curr.jumlah_pembayaran), 0) || 0)) <= 0,
                                        'bg-yellow-100 text-yellow-800': (item.total_transaksi - (item.pembayaran_layanans?.reduce((acc, curr) => acc + Number(curr.jumlah_pembayaran), 0) || 0)) > 0
                                     }" class="px-2 py-1 rounded text-xs font-semibold uppercase" x-text="(item.total_transaksi - (item.pembayaran_layanans?.reduce((acc, curr) => acc + Number(curr.jumlah_pembayaran), 0) || 0)) <= 0 ? 'LUNAS' : 'BELUM LUNAS'"></span>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a :href="`/transaksi-layanan/${item.id}`" class="text-gray-500 hover:text-gray-700" title="Lihat">
                                        ...
                                        <template x-if="canEdit">
                                            <a :href="`/transaksi-layanan/${item.id}/edit`" class="text-blue-500 hover:text-blue-700" title="Edit">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                    <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                                                </svg>
                                            </a>
                                        </template>
                                        <a :href="'/transaksi-layanan/pdf/' + item.id" class="text-green-500 hover:text-green-700" title="Download PDF">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                                        </a>
                                        <a :href="'/transaksi-layanan/print/' + item.id" target="_blank" class="text-indigo-500 hover:text-indigo-700" title="Print Invoice">
