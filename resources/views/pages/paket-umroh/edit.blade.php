...
<div class="grid grid-cols-12 gap-4 md:gap-6">
    <div class="col-span-12">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="kode_paket" value="{{ $paketUmroh->kode_paket }}" readonly class="w-full rounded-lg border border-gray-300 bg-gray-100 px-4 py-2.5 text-sm text-gray-500" />
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Nama Paket</label>
                    <input type="text" name="nama_paket" value="{{ old('nama_paket', $paketUmroh->nama_paket) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Tanggal Keberangkatan</label>
                    <input type="date" name="tanggal_keberangkatan" value="{{ old('tanggal_keberangkatan', $paketUmroh->tanggal_keberangkatan) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Jumlah Hari</label>
                    <input type="number" name="jumlah_hari" value="{{ old('jumlah_hari', $paketUmroh->jumlah_hari) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="number" name="kuota_jamaah" value="{{ old('kuota_jamaah', $paketUmroh->kuota_jamaah) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                ...
                <div>
                    ...
                    <select name="maskapai_id" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                        @foreach($maskapais as $maskapai)
                            <option value="{{ $maskapai->id }}" {{ $paketUmroh->maskapai_id == $maskapai->id ? 'selected' : '' }}>{{ $maskapai->nama_maskapai }}</option>
                        @endforeach
                    </select>
                </div>
                 <div>
                 ...
                 <div>
                    ...
                    <select name="lokasi_keberangkatan" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                        @foreach(['Jakarta', 'Surabaya', 'Makasar', 'Balikpapan', 'Medan', 'Pekanbaru', 'Denpasar', 'Lombok', 'Jambi', 'Batam'] as $loc)
                            <option value="{{ $loc }}" {{ $paketUmroh->lokasi_keberangkatan == $loc ? 'selected' : '' }}>{{ $loc }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                 <div>
                    ...
                    <input type="text" name="jenis_paket_1" value="{{ old('jenis_paket_1', $paketUmroh->jenis_paket_1) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                 <div>
                    ...
                    <select name="hotel_mekkah_1" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                        @foreach($hotelsMekkah as $hotel)
                            <option value="{{ $hotel->id }}" {{ $paketUmroh->hotel_mekkah_1 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                        @endforeach
                    </select>
                </div>
                 <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Hotel Madinah</label>
                     <select name="hotel_madinah_1" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                        @foreach($hotelsMadinah as $hotel)
                            <option value="{{ $hotel->id }}" {{ $paketUmroh->hotel_madinah_1 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                        @endforeach
                    </select>
                </div>
                 <div>
                    ...
                    <select name="hotel_transit_1" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                        ...
                        @foreach($hotelsTransit as $hotel)
                            <option value="{{ $hotel->id }}" {{ $paketUmroh->hotel_transit_1 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga HPP</label>
                    <input type="number" name="harga_hpp_1" value="{{ old('harga_hpp_1', $paketUmroh->harga_hpp_1) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga Quad</label>
                    <input type="number" name="harga_quad_1" value="{{ old('harga_quad_1', $paketUmroh->harga_quad_1) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga Triple</label>
                    <input type="number" name="harga_triple_1" value="{{ old('harga_triple_1', $paketUmroh->harga_triple_1) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga Double</label>
                    <input type="number" name="harga_double_1" value="{{ old('harga_double_1', $paketUmroh->harga_double_1) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" required />
                </div>
            </div>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                 <div>
                    ...
                    <input type="text" name="jenis_paket_2" value="{{ old('jenis_paket_2', $paketUmroh->jenis_paket_2) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" />
                </div>
                 <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Hotel Mekkah</label>
                    <select name="hotel_mekkah_2" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                        ...
                        @foreach($hotelsMekkah as $hotel)
                            <option value="{{ $hotel->id }}" {{ $paketUmroh->hotel_mekkah_2 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                        @endforeach
                    </select>
                </div>
                 <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Hotel Madinah</label>
                     <select name="hotel_madinah_2" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                         <option value="">Pilih Hotel</option>
                        @foreach($hotelsMadinah as $hotel)
                            <option value="{{ $hotel->id }}" {{ $paketUmroh->hotel_madinah_2 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                        @endforeach
                    </select>
                </div>
                 <div>
                    ...
                    <select name="hotel_transit_2" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">
                        <option value="">Tidak Ada</option>
                        @foreach($hotelsTransit as $hotel)
                            <option value="{{ $hotel->id }}" {{ $paketUmroh->hotel_transit_2 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga HPP</label>
                    <input type="number" name="harga_hpp_2" value="{{ old('harga_hpp_2', $paketUmroh->harga_hpp_2) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" />
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga Quad</label>
                    <input type="number" name="harga_quad_2" value="{{ old('harga_quad_2', $paketUmroh->harga_quad_2) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" />
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga Triple</label>
                    <input type="number" name="harga_triple_2" value="{{ old('harga_triple_2', $paketUmroh->harga_triple_2) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" />
                </div>
                <div>
                     <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Harga Double</label>
                    <input type="number" name="harga_double_2" value="{{ old('harga_double_2', $paketUmroh->harga_double_2) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm" />
                </div>
            </div>
        </div>
         ...
         <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                 <div>
                    ...
                    <textarea name="termasuk_paket" rows="4" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">{{ old('termasuk_paket', $paketUmroh->termasuk_paket) }}</textarea>
                </div>
                 <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Tidak Termasuk Paket (Exclude)</label>
                    <textarea name="tidak_termasuk_paket" rows="4" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">{{ old('tidak_termasuk_paket', $paketUmroh->tidak_termasuk_paket) }}</textarea>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Syarat & Ketentuan</label>
                    <textarea name="syarat_ketentuan" rows="4" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">{{ old('syarat_ketentuan', $paketUmroh->syarat_ketentuan) }}</textarea>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Catatan Paket</label>
                    <textarea name="catatan_paket" rows="2" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm">{{ old('catatan_paket', $paketUmroh->catatan_paket) }}</textarea>
                </div>
                <div>
        ...
        <div class="mt-8 flex justify-end gap-3">
            <a href="{{ route('paket-umroh') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Batal</a>
