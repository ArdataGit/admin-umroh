...
@section('content')
<x-common.page-breadcrumb pageTitle="Setoran Tabungan Haji" :breadcrumbs="[
...
<div class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        <h3 class="mb-4 text-xl font-semibold text-gray-800 dark:text-white">Informasi Jamaah & Tabungan</h3>
        <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
             <div class="space-y-1">
                <p class="text-sm text-gray-500">Nama Jamaah</p>
                <p class="font-medium text-gray-800 dark:text-white">{{ $tabungan->jamaah->kode_jamaah }} / {{ $tabungan->jamaah->nama_lengkap }}</p>
            </div>
             <div class="space-y-1">
                <p class="text-sm text-gray-500">Tanggal Lahir / Umur</p>
                <p class="font-medium text-gray-800 dark:text-white">{{ \Carbon\Carbon::parse($tabungan->jamaah->tanggal_lahir)->translatedFormat('d F Y') }} / {{ $umur }} Tahun</p>
            </div>
             <div class="space-y-1">
                <p class="text-sm text-gray-500">Kontak Jamaah / Kota</p>
                <p class="font-medium text-gray-800 dark:text-white">{{ $tabungan->jamaah->kontak_jamaah }} / {{ $tabungan->jamaah->kabupaten_kota }}</p>
            </div>
             <div class="space-y-1">
                <p class="text-sm text-gray-500">Total Penarikan</p>
                <p class="font-medium text-red-600">Rp {{ number_format($total_penarikan, 0, ',', '.') }}</p>
            </div>
             <div class="space-y-1">
                <p class="text-sm text-gray-500">Saldo Tabungan</p>
                <p class="font-medium text-green-600">Rp {{ number_format($tabungan->setoran_tabungan, 0, ',', '.') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="col-span-12" x-data='{
      transactions: @json($tabungan->transaksiTabunganHajis),
      jamaahName: "{{ $tabungan->jamaah->nama_lengkap }}",
      currentPage: 1,
      itemsPerPage: 10,
      sortField: "",
      sortDirection: "desc",
      ...
      confirmDelete() {
        ...
        fetch(`/setoran-haji/transaksi/${this.deleteTarget.id}`, {
          method: "DELETE",
          headers: { "X-CSRF-TOKEN": document.querySelector("meta[name=csrf-token]").content, "Accept": "application/json" }
            ...
            if (d.success) { 
                ...
                window.location.reload(); // Reload to update balance header
            }
        ...
        <div class="rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            <div class="flex flex-col gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-700 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-4">
                    <h3 class="text-lg font-semibold text-gray-800 dark:text-white">Riwayat Transaksi</h3>
                    <a href="{{ route('setoran-haji.create', $tabungan->id) }}" class="rounded-lg bg-blue-500 px-4 py-2 text-sm font-medium text-white hover:bg-blue-600">
                        Tambah Data
                    </a>
                </div>
                 <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                    ...
                    <div class="relative">
                        <input type="text" x-model="searchQuery" @input="currentPage = 1" placeholder="Cari Transaksi..." class="h-10 w-full rounded-lg border border-gray-300 px-4 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white sm:w-64"/>
                    </div>
                </div>
            </div>
                ...
                <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
                    <thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                            ...
                            <th class="px-6 py-3">Status Setoran</th>
                            <th class="px-6 py-3">Kode Referensi</th>
                    ...
                    <tbody>
                        <template x-for="(trx, index) in paginatedTransactions" :key="trx.id">
                            <tr class="border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600">
                                ...
                                <td class="px-6 py-4" x-text="jamaahName"></td>
                                ...
                                <td class="px-6 py-4" x-text="trx.kode_referensi || '-'"></td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        ...
                                        <a :href="`/setoran-haji/transaksi/${trx.id}/edit`" class="text-yellow-500 hover:text-yellow-700" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
