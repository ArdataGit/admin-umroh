...
@section('content')
<x-common.page-breadcrumb pageTitle="Edit Paket Haji" :breadcrumbs="[
...
<div class="grid grid-cols-12 gap-4 md:gap-6">
    <div class="col-span-12">
        <form action="{{ route('paket-haji.update', $paketHaji->id) }}" method="POST" enctype="multipart/form-data"
            x-data="{ 
                formatNumber(num) {
                    if (!num && num !== 0) return '';
                    return new Intl.NumberFormat('id-ID').format(Math.round(num));
                },
                maskapai_price: {{ $paketHaji->maskapai->harga_tiket ?? 0 }},
                service_prices: {
                    @foreach($paketHaji->layanans as $layanan)
                        {{ $layanan->id }}: {{ $layanan->harga_jual }},
                    @endforeach
                },
                v1: {
                    mekkah_p: {{ $paketHaji->hotelMekkah1->harga_hotel ?? 0 }},
                    mekkah_m: {{ $paketHaji->hotelMekkah1->biaya_makan ?? 0 }},
                    mekkah_d: {{ old('hari_mekkah_1', $paketHaji->hari_mekkah_1 ?? 0) }},
                    madinah_p: {{ $paketHaji->hotelMadinah1->harga_hotel ?? 0 }},
                    madinah_m: {{ $paketHaji->hotelMadinah1->biaya_makan ?? 0 }},
                    madinah_d: {{ old('hari_madinah_1', $paketHaji->hari_madinah_1 ?? 0) }},
                    transit_p: {{ $paketHaji->hotelTransit1->harga_hotel ?? 0 }},
                    transit_m: {{ $paketHaji->hotelTransit1->biaya_makan ?? 0 }},
                    transit_d: {{ old('hari_transit_1', $paketHaji->hari_transit_1 ?? 0) }},
                    hpp_q: {{ old('hpp_quad1', $paketHaji->hpp_quad1 ?? 0) }},
                    hpp_t: {{ old('hpp_triple1', $paketHaji->hpp_triple1 ?? 0) }},
                    hpp_d: {{ old('hpp_double1', $paketHaji->hpp_double1 ?? 0) }},
                    quad_jual: {{ old('harga_quad_1', $paketHaji->harga_quad_1 ?? 0) }},
                    triple_jual: {{ old('harga_triple_1', $paketHaji->harga_triple_1 ?? 0) }},
                    double_jual: {{ old('harga_double_1', $paketHaji->harga_double_1 ?? 0) }},
                    is_include_makan: {{ old('is_include_makan_1', $paketHaji->is_include_makan_1 ?? 'true') == 'true' || old('is_include_makan_1', $paketHaji->is_include_makan_1) == 1 ? 'true' : 'false' }}
                },
                v2: {
                    mekkah_p: {{ $paketHaji->hotelMekkah2->harga_hotel ?? 0 }},
                    mekkah_m: {{ $paketHaji->hotelMekkah2->biaya_makan ?? 0 }},
                    mekkah_d: {{ old('hari_mekkah_2', $paketHaji->hari_mekkah_2 ?? 0) }},
                    madinah_p: {{ $paketHaji->hotelMadinah2->harga_hotel ?? 0 }},
                    madinah_m: {{ $paketHaji->hotelMadinah2->biaya_makan ?? 0 }},
                    madinah_d: {{ old('hari_madinah_2', $paketHaji->hari_madinah_2 ?? 0) }},
                    transit_p: {{ $paketHaji->hotelTransit2->harga_hotel ?? 0 }},
                    transit_m: {{ $paketHaji->hotelTransit2->biaya_makan ?? 0 }},
                    transit_d: {{ old('hari_transit_2', $paketHaji->hari_transit_2 ?? 0) }},
                    hpp_q: {{ old('hpp_quad2', $paketHaji->hpp_quad2 ?? 0) }},
                    hpp_t: {{ old('hpp_triple2', $paketHaji->hpp_triple2 ?? 0) }},
                    hpp_d: {{ old('hpp_double2', $paketHaji->hpp_double2 ?? 0) }},
                    quad_jual: {{ old('harga_quad_2', $paketHaji->harga_quad_2 ?? 0) }},
                    triple_jual: {{ old('harga_triple_2', $paketHaji->harga_triple_2 ?? 0) }},
                    double_jual: {{ old('harga_double_2', $paketHaji->harga_double_2 ?? 0) }},
                    is_include_makan: {{ old('is_include_makan_2', $paketHaji->is_include_makan_2 ?? 'true') == 'true' || old('is_include_makan_2', $paketHaji->is_include_makan_2) == 1 ? 'true' : 'false' }}
                },
                get serviceTotal() {
                ...
                calculateHPP(variant) {
                    ...
                    const mekkah_m = ctx.is_include_makan ? (parseFloat(ctx.mekkah_m) || 0) : 0;
                    const mekkah_d = parseFloat(ctx.mekkah_d) || 0;
                    
                    const madinah_p = parseFloat(ctx.madinah_p) || 0;
                    const madinah_m = ctx.is_include_makan ? (parseFloat(ctx.madinah_m) || 0) : 0;
                    const madinah_d = parseFloat(ctx.madinah_d) || 0;
                    
                    const transit_p = parseFloat(ctx.transit_p) || 0;
                    const transit_m = ctx.is_include_makan ? (parseFloat(ctx.transit_m) || 0) : 0;
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="kode_paket" value="{{ $paketHaji->kode_paket }}" readonly class="w-full rounded-lg border border-gray-300 bg-gray-100 px-4 py-2.5 text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400" />
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Nama Paket</label>
                    <input type="text" name="nama_paket" value="{{ old('nama_paket', $paketHaji->nama_paket) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" required />
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Tanggal Keberangkatan</label>
                    <input type="date" name="tanggal_keberangkatan" value="{{ old('tanggal_keberangkatan', $paketHaji->tanggal_keberangkatan) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" required />
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Jumlah Hari</label>
                    <input type="number" name="jumlah_hari" value="{{ old('jumlah_hari', $paketHaji->jumlah_hari) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" required />
                </div>
                <div>
                    ...
                    <select name="status_paket" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                        <option value="active" {{ $paketHaji->status_paket == 'active' ? 'selected' : '' }}>Active</option>
                        <option value="completed" {{ $paketHaji->status_paket == 'completed' ? 'selected' : '' }}>Completed</option>
                    </select>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Kuota Jamaah (Pax)</label>
                    <input type="number" name="kuota_jamaah" value="{{ old('kuota_jamaah', $paketHaji->kuota_jamaah) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" required />
                </div>
                ...
                <div>
                    ...
                    <select name="maskapai_id" @change="maskapai_price = $el.options[$el.selectedIndex].dataset.price || 0; calculateHPP(1); calculateHPP(2)" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                        @foreach($maskapais as $maskapai)
                            <option value="{{ $maskapai->id }}" data-price="{{ $maskapai->harga_tiket }}" {{ $paketHaji->maskapai_id == $maskapai->id ? 'selected' : '' }}>{{ $maskapai->nama_maskapai }}</option>
                        @endforeach
                    </select>
                </div>
                 <div>
                    ...
                    <select name="rute_penerbangan" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                        <option value="direct" {{ $paketHaji->rute_penerbangan == 'direct' ? 'selected' : '' }}>Direct</option>
                        <option value="transit" {{ $paketHaji->rute_penerbangan == 'transit' ? 'selected' : '' }}>Transit</option>
                    </select>
                </div>
                 <div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                @php
                    $selectedLayanans = old('layanan_ids', $paketHaji->layanans->pluck('id')->toArray());
                @endphp
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                 <div>
                    ...
                    <input type="text" name="jenis_paket_1" value="{{ old('jenis_paket_1', $paketHaji->jenis_paket_1) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" required />
                </div>
                    ...
                    <div>
                        ...
                        <select name="hotel_mekkah_1" @change="v1.mekkah_p = $el.options[$el.selectedIndex].dataset.price || 0; v1.mekkah_m = $el.options[$el.selectedIndex].dataset.meal || 0; calculateHPP(1)" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            @foreach($hotelsMekkah as $hotel)
                                <option value="{{ $hotel->id }}" data-price="{{ $hotel->harga_hotel }}" data-meal="{{ $hotel->biaya_makan }}" {{ $paketHaji->hotel_mekkah_1 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="hotel_madinah_1" @change="v1.madinah_p = $el.options[$el.selectedIndex].dataset.price || 0; v1.madinah_m = $el.options[$el.selectedIndex].dataset.meal || 0; calculateHPP(1)" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" required>
                            @foreach($hotelsMadinah as $hotel)
                                <option value="{{ $hotel->id }}" data-price="{{ $hotel->harga_hotel }}" data-meal="{{ $hotel->biaya_makan }}" {{ $paketHaji->hotel_madinah_1 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="hotel_transit_1" @change="v1.transit_p = $el.options[$el.selectedIndex].dataset.price || 0; v1.transit_m = $el.options[$el.selectedIndex].dataset.meal || 0; calculateHPP(1)" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            ...
                            @foreach($hotelsTransit as $hotel)
                                <option value="{{ $hotel->id }}" data-price="{{ $hotel->harga_hotel }}" data-meal="{{ $hotel->biaya_makan }}" {{ $paketHaji->hotel_transit_1 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                 <div>
                    ...
                    <input type="text" name="jenis_paket_2" value="{{ old('jenis_paket_2', $paketHaji->jenis_paket_2) }}" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Hotel Mekkah</label>
                        <select name="hotel_mekkah_2" @change="v2.mekkah_p = $el.options[$el.selectedIndex].dataset.price || 0; v2.mekkah_m = $el.options[$el.selectedIndex].dataset.meal || 0; calculateHPP(2)" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            ...
                            @foreach($hotelsMekkah as $hotel)
                                <option value="{{ $hotel->id }}" data-price="{{ $hotel->harga_hotel }}" data-meal="{{ $hotel->biaya_makan }}" {{ $paketHaji->hotel_mekkah_2 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Hotel Madinah</label>
                         <select name="hotel_madinah_2" @change="v2.madinah_p = $el.options[$el.selectedIndex].dataset.price || 0; v2.madinah_m = $el.options[$el.selectedIndex].dataset.meal || 0; calculateHPP(2)" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                             <option value="">Pilih Hotel</option>
                            @foreach($hotelsMadinah as $hotel)
                                <option value="{{ $hotel->id }}" data-price="{{ $hotel->harga_hotel }}" data-meal="{{ $hotel->biaya_makan }}" {{ $paketHaji->hotel_madinah_2 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="hotel_transit_2" @change="v2.transit_p = $el.options[$el.selectedIndex].dataset.price || 0; v2.transit_m = $el.options[$el.selectedIndex].dataset.meal || 0; calculateHPP(2)" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">
                            <option value="">Tidak Ada</option>
                            @foreach($hotelsTransit as $hotel)
                                <option value="{{ $hotel->id }}" data-price="{{ $hotel->harga_hotel }}" data-meal="{{ $hotel->biaya_makan }}" {{ $paketHaji->hotel_transit_2 == $hotel->id ? 'selected' : '' }}>{{ $hotel->nama_hotel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
         ...
         <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900 mb-6">
            ...
            <div class="grid grid-cols-1 gap-6">
                 <div>
                    ...
                    <textarea name="termasuk_paket" rows="4" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('termasuk_paket', $paketHaji->termasuk_paket) }}</textarea>
                </div>
                 <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Tidak Termasuk Paket (Exclude)</label>
                    <textarea name="tidak_termasuk_paket" rows="4" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('tidak_termasuk_paket', $paketHaji->tidak_termasuk_paket) }}</textarea>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Syarat & Ketentuan</label>
                    <textarea name="syarat_ketentuan" rows="4" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('syarat_ketentuan', $paketHaji->syarat_ketentuan) }}</textarea>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Catatan Paket</label>
                    <textarea name="catatan_paket" rows="2" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('catatan_paket', $paketHaji->catatan_paket) }}</textarea>
                </div>
                <div>
                    <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Foto Brosur</label>
                    @if($paketHaji->foto_brosur)
                         <img src="{{ asset('storage/' . $paketHaji->foto_brosur) }}" class="mb-2 h-20 rounded-md object-cover">
                    @endif
        ...
        <div class="mt-8 flex justify-end gap-3">
            <a href="{{ route('paket-haji') }}" class="rounded-lg border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700">Batal</a>
