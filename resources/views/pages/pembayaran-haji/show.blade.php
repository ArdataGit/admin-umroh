...
@section('content')
<x-common.page-breadcrumb pageTitle="Detail Pembayaran Haji" :breadcrumbs="[
...
<div class="col-span-12">
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 gap-2 text-sm">
                    <div class="grid grid-cols-3">
                        ...
                        <span class="col-span-2 font-medium text-gray-800 dark:text-gray-200">: {{ $pembayaran->customerHaji->jamaah->nama_jamaah }}</span>
                    </div>
                    <div class="grid grid-cols-3">
                        <span class="text-gray-500">Paket</span>
                        <span class="col-span-2 font-medium text-gray-800 dark:text-gray-200">: {{ $pembayaran->customerHaji->keberangkatanHaji->paketHaji->nama_paket ?? '-' }}</span>
                    </div>
                     <div class="grid grid-cols-3">
                        <span class="text-gray-500">Keberangkatan</span>
                        <span class="col-span-2 font-medium text-gray-800 dark:text-gray-200">: {{ $pembayaran->customerHaji->keberangkatanHaji->nama_keberangkatan }}</span>
                    </div>
                    <div class="grid grid-cols-3">
                        <span class="text-gray-500">Total Tagihan</span>
                        <span class="col-span-2 font-medium text-gray-800 dark:text-gray-200">: Rp {{ number_format($pembayaran->customerHaji->total_tagihan, 0, ',', '.') }}</span>
                    </div>
                     <div class="grid grid-cols-3">
                        <span class="text-gray-500">Sisa Tagihan</span>
                        <span class="col-span-2 font-medium text-red-600">: Rp {{ number_format($pembayaran->customerHaji->sisa_tagihan, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-8 flex justify-end gap-3 border-t border-gray-100 pt-6 dark:border-gray-800">
            <a href="{{ route('pembayaran-haji.index') }}" class="rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">
                Kembali
            </a>
            <a href="{{ route('pembayaran-haji.edit', $pembayaran->id) }}" class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
                Edit Pembayaran
            </a>
        </div>
