...
@section('content')
<x-common.page-breadcrumb pageTitle="Data Pembayaran Umroh" :breadcrumbs="[
    ['label' => 'Pembayaran Umroh', 'url' => '#']
]" />
    ...
    <div class="col-span-12" x-data='{
        ...
        get filteredPembayarans() {
            ...
            return this.pembayarans.filter(item => {
                return (item.kode_transaksi && item.kode_transaksi.toLowerCase().includes(query)) ||
                       (item.customer_umroh.jamaah.nama_jamaah && item.customer_umroh.jamaah.nama_jamaah.toLowerCase().includes(query)) ||
                       (item.customer_umroh.keberangkatan_umroh.nama_keberangkatan && item.customer_umroh.keberangkatan_umroh.nama_keberangkatan.toLowerCase().includes(query));
            });
        },
        get sortedPembayarans() {
            ...
            return [...this.filteredPembayarans].sort((a, b) => {
                ...
                // Handle nested properties if needed, simplified for direct access
                // For nested like jamaah name, we might need to map them before or handle logic here
                if(this.sortField === "nama_jamaah") {
                    aVal = a.customer_umroh.jamaah.nama_jamaah;
                    bVal = b.customer_umroh.jamaah.nama_jamaah;
                } else if (this.sortField === "nama_keberangkatan") {
                    aVal = a.customer_umroh.keberangkatan_umroh.nama_keberangkatan;
                    bVal = b.customer_umroh.keberangkatan_umroh.nama_keberangkatan;
                }
                    ...
                    <tbody>
                         <template x-for="(item, index) in paginatedPembayarans" :key="item.id">
                            <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-4 py-4">
                                     <p class="font-medium text-gray-800 text-theme-sm dark:text-white/90" x-text="item.customer_umroh.jamaah.nama_jamaah"></p>
                                </td>
                                <td class="px-4 py-4">
                                     <a :href="'{{ route('customer-umroh.index', ':id') }}'.replace(':id', item.customer_umroh.keberangkatan_umroh.id)" class="font-medium text-blue-600 hover:text-blue-800 text-theme-sm hover:underline" x-text="item.customer_umroh.keberangkatan_umroh.nama_keberangkatan"></a>
                                </td>
                                ...
                                <td class="px-4 py-4 text-center">
                                    <div class="flex justify-center gap-2">
                                        <a :href="'{{ route('pembayaran-umroh.history', ':id') }}'.replace(':id', item.customer_umroh_id)" class="text-green-500 hover:text-green-700 dark:hover:text-green-300" title="History & Add Payment">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                        </a>
                                        <a :href="'{{ route('pembayaran-umroh.detail', ':id') }}'.replace(':id', item.id)" class="text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" title="Detail">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                        </a>
                                         <a :href="'{{ route('pembayaran-umroh.print-pdf', ':id') }}'.replace(':id', item.id)" target="_blank" class="text-blue-500 hover:text-blue-700 dark:hover:text-blue-300" title="Print Invoice">
