...
@section('content')
<x-common.page-breadcrumb pageTitle="Riwayat Pembayaran" :breadcrumbs="[
...
<div class="space-y-6">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        <div class="flex flex-col md:flex-row justify-between md:items-center gap-4">
            <div>
                <h3 class="text-xl font-semibold text-gray-800 dark:text-white">{{ $customerUmroh->jamaah->nama_jamaah }}</h3>
                <p class="text-sm text-gray-500 dark:text-gray-400">
                    Paket: {{ $customerUmroh->keberangkatanUmroh->paketUmroh->nama_paket ?? '-' }} | 
                    Keberangkatan: {{ $customerUmroh->keberangkatanUmroh->nama_keberangkatan }}
                </p>
            </div>
            <div class="flex flex-col items-end">
                 <div class="text-sm text-gray-500">Total Tagihan</div>
                 <div class="text-lg font-bold text-gray-800 dark:text-white">Rp {{ number_format($customerUmroh->total_tagihan, 0, ',', '.') }}</div>
                 <div class="text-sm {{ $customerUmroh->sisa_tagihan > 0 ? 'text-red-600' : 'text-green-600' }} font-medium">
                    Sisa: Rp {{ number_format($customerUmroh->sisa_tagihan, 0, ',', '.') }}
                 </div>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-center justify-between p-6 border-b border-gray-100 dark:border-gray-800">
            ...
            @if($canCreate)
            <a href="{{ route('pembayaran-umroh.create-payment', $customerUmroh->id) }}" class="rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700 transition">
                + Tambah Pembayaran
            </a>
            @endif
        </div>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4 text-center">
                             <a href="{{ route('pembayaran-umroh.detail', $pembayaran->id) }}" class="text-blue-600 hover:text-blue-800 mr-2" title="Detail">
                                <svg class="w-5 h-5 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                             </a>
                             @if($canEdit)
                             <a href="{{ route('pembayaran-umroh.edit', $pembayaran->id) }}" class="text-gray-600 hover:text-gray-800" title="Edit">
