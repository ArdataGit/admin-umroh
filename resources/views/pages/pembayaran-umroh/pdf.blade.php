...
<body>
    ...
    <div class="content">
        <div class="header-title" style="margin-top: 0px;">INVOICE PEMBAYARAN UMROH</div>
        ...
        <table class="address-table">
            <tr>
                ...
                <td>
                    ...
                    <div class="company-name">{{ $customerUmroh->jamaah->nama_jamaah }}</div>
                    <div>
                        Kode Jamaah: {{ $customerUmroh->jamaah->kode_jamaah }}<br>
                        Alamat: {{ $customerUmroh->jamaah->alamat_lengkap ?? '-' }}<br>
                        Telp: {{ $customerUmroh->jamaah->nomor_hp ?? '-' }}
                    </div>
                </td>
            </tr>
        </table>
    ...
    <table class="main-table">
        ...
        <tbody>
            <tr>
                <td>
                    <strong>{{ $customerUmroh->keberangkatanUmroh->paketUmroh->nama_paket }}</strong><br>
                    Kode: {{ $customerUmroh->keberangkatanUmroh->paketUmroh->kode_paket }}
                </td>
                <td style="text-align: center;">
                    {{ \Carbon\Carbon::parse($customerUmroh->keberangkatanUmroh->tanggal_keberangkatan)->format('d/m/Y') }}<br>
                    <small>{{ $customerUmroh->keberangkatanUmroh->nama_keberangkatan }}</small>
                </td>
                <td style="text-align: center;" class="capitalize">{{ $customerUmroh->tipe_kamar }}</td>
                <td style="text-align: center;">{{ $customerUmroh->jumlah_jamaah }} Pax</td>
                <td style="text-align: right;">Rp {{ number_format($customerUmroh->total_tagihan, 0, ',', '.') }}</td>
            </tr>
        </tbody>
    </table>
        ...
        <div class="summary-section">
            <table class="summary-table">
                <tr>
                    ...
                    <td class="value">Rp {{ number_format($customerUmroh->harga_paket * $customerUmroh->jumlah_jamaah, 0, ',', '.') }}</td>
                </tr>
                @if($customerUmroh->diskon > 0)
                <tr>
                    <td class="label">Diskon</td>
                    <td class="value">(-) Rp {{ number_format($customerUmroh->diskon, 0, ',', '.') }}</td>
                </tr>
                @endif
                <tr>
                    <td class="label grand-total">Total Tagihan</td>
                    <td class="value grand-total">Rp {{ number_format($customerUmroh->total_tagihan, 0, ',', '.') }}</td>
                </tr>
                <tr>
    ...
    <div class="payment-status-section">
        <table class="status-table">
            ...
            <tbody>
                <tr>
                    <td>Rp {{ number_format($customerUmroh->total_tagihan, 0, ',', '.') }}</td>
