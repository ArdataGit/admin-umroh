...
@section('content')
<x-common.page-breadcrumb pageTitle="Tambah Pembayaran" :breadcrumbs="[
    ['label' => 'Pembayaran Umroh', 'url' => route('pembayaran-umroh.index')],
    ['label' => 'History', 'url' => route('pembayaran-umroh.history', $customerUmroh->id)],
    ['label' => 'Tambah', 'url' => '#']
]" />
...
<div class="col-span-12">
    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-theme-xs dark:border-gray-800 dark:bg-gray-900">
        ...
        <form action="{{ route('pembayaran-umroh.store-payment', $customerUmroh->id) }}" method="POST">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $customerUmroh->jamaah->nama_jamaah }}" readonly class="w-full rounded-lg border border-gray-300 bg-gray-100 px-4 py-2.5 text-sm text-gray-500" />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Total Tagihan</label>
                        <input type="text" value="Rp {{ number_format($customerUmroh->total_tagihan, 0, ',', '.') }}" readonly class="w-full rounded-lg border border-gray-300 bg-gray-100 px-4 py-2.5 text-sm text-gray-500" />
                    </div>
                     <div>
                        <label class="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-400">Sisa Tagihan Saat Ini</label>
                        <div class="relative">
                            <input type="text" value="Rp {{ number_format($customerUmroh->sisa_tagihan, 0, ',', '.') }}" readonly class="w-full rounded-lg border border-red-200 bg-red-50 px-4 py-2.5 text-sm text-red-600 font-bold" />
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div x-data="{ amount: {{ $customerUmroh->sisa_tagihan }} }">
                    ...
                    <div class="pt-4 flex justify-end gap-3">
                        <a href="{{ route('pembayaran-umroh.history', $customerUmroh->id) }}" class="rounded-lg border border-gray-300 px-6 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800">
                            Batal
                        </a>
