...
@section('content')
<x-common.page-breadcrumb pageTitle="Laporan Haji" :breadcrumbs="[
    ['label' => 'Laporan Haji', 'url' => '#']
]" />
        ...
        get filteredItems() {
            ...
            return this.items.filter(item => {
                return (item.nama_keberangkatan && item.nama_keberangkatan.toLowerCase().includes(query)) ||
                       (item.paket_haji && item.paket_haji.nama_paket && item.paket_haji.nama_paket.toLowerCase().includes(query));
            });
        },
        get sortedItems() {
            ...
            return [...this.filteredItems].sort((a, b) => {
                ...
                if (this.sortField === "nama_paket") {
                     aVal = a.paket_haji ? a.paket_haji.nama_paket : "";
                     bVal = b.paket_haji ? b.paket_haji.nama_paket : "";
                }
                    ...
                    <tbody>
                          <template x-for="(item, index) in paginatedItems" :key="item.id">
                            <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                ...
                                <td class="px-4 py-4 text-center">
                                     <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300" x-text="item.customer_hajis_count"></span>
                                </td>
                                <td class="px-4 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <a :href="'/laporan-haji/' + item.id" class="inline-flex items-center gap-2 rounded-lg bg-gray-100 px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-200 dark:bg-white/5 dark:text-white/80 dark:hover:bg-white/10">
                         ...
                         <tr x-show="filteredItems.length === 0">
                            <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                                Tidak ada data keberangkatan haji yang ditemukan.
                            </td>
                         </tr>
                    </tbody>
                </table>
