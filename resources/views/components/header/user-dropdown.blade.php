<div class="relative" x-data="{
    ...
    >
        <span class="mr-3 overflow-hidden rounded-full h-11 w-11 border border-gray-200 dark:border-gray-700">
            @if(auth()->user()->avatar)
                <img src="{{ asset('avatars/'.auth()->user()->avatar) }}" alt="User" class="h-full w-full object-cover" />
            @else
                <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=random" alt="User" class="h-full w-full object-cover" />
            @endif
        </span>
